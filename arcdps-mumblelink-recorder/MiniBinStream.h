...
// version 1.0.4   : Fixed file_istream's seekg() and added writeat() to mem_ostream and memfile_ostream. Thanks
// Festering from CodeProject.
...
namespace simple
{
    ...
    using number_type = typename std::conditional<
        ...
        typename std::conditional<
            sizeof(T) == 2, SizeOf2,
            typename std::conditional<sizeof(T) == 4, SizeOf4,
                                      typename std::conditional<sizeof(T) == 8, SizeOf8, UnknownSize>::type>::type>::
        type>::type;
    ...
    public:
        ...
        file_istream(const char *file) :
            input_file_ptr(nullptr), file_size(0L), read_length(0L) { open(file); }
#ifdef _MSC_VER
        ...
        bool is_open() { return (input_file_ptr != nullptr); }
        long file_length() const { return file_size; }
        ...
        long tellg() const { return std::ftell(input_file_ptr); }
    ...
    public:
        ...
        mem_istream(const char *mem, size_t size) { open(mem, size); }
        ...
        void close() { m_vec.clear(); }
        bool eof() const { return m_index >= m_vec.size(); }
        std::ifstream::pos_type tellg() const { return m_index; }
        ...
        const std::vector<char> &get_internal_vec() { return m_vec; }
    ...
    public:
        ...
        ptr_istream(const char *mem, size_t size) :
            m_arr(nullptr), m_size(0), m_index(0) { open(mem, size); }
        ...
        bool eof() const { return m_index >= m_size; }
        std::ifstream::pos_type tellg() const { return m_index; }
    ...
    public:
        ...
        memfile_istream(const char *file) :
            m_arr(nullptr), m_size(0), m_index(0) { open(file); }
#ifdef _MSC_VER
        memfile_istream(const wchar_t *file) :
            m_arr(nullptr), m_size(0), m_index(0) { open(file); }
#endif
        ~memfile_istream() { close(); }
        ...
        bool is_open() { return (m_arr != nullptr); }
        long file_length() const { return m_size; }
        bool eof() const { return m_index >= m_size; }
        std::ifstream::pos_type tellg() const { return m_index; }
    ...
    public:
        ...
        file_ostream(const char *file) :
            output_file_ptr(nullptr) { open(file); }
#ifdef _MSC_VER
...
#endif
        void flush() { std::fflush(output_file_ptr); }
        ...
        bool is_open() { return output_file_ptr != nullptr; }
    ...
    public:
        ...
        void write(const std::vector<char> &vec) { m_vec.insert(m_vec.end(), vec.begin(), vec.end()); }
